...
- [x] **Analytics — both switches are on (Sep 15 2026).** Cloudflare Web Analytics enabled by the owner in the Pages project's Metrics tab. Google Search Console: URL-prefix property `https://vertical-agent-solutions.pages.dev/` added and **verified by HTML tag** (token committed as `GSC_VERIFICATION` in `src/consts.ts`; the HTML-file method fails on Pages because `/google….html` is 308-redirected to a clean URL). `sitemap-index.xml` submitted the same day; its first status was "Couldn't fetch", which is the normal placeholder before Google's first crawl of a new property. **Recheck in a few days**: Sitemaps should show 90+ discovered pages, and Performance will say whether anything is indexed. Still to do: Bing Webmaster Tools one-click import from Search Console. When the custom domain lands, add it as a second URL-prefix property; the same tag verifies it.
