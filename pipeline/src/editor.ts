 ...
 * reports what it did to Telegram as a plain notification — no buttons, no
 * interactive actions. Every action is one commit; revert it in git if needed.
 *
 ...
 *      SITE_URL (optional), EDITOR_DRY_RUN=1 (optional — no push/Telegram).
 */
...
async function decide(posts: Post[], log: LogEntry[]): Promise<Decision> {
...
async function execute(decision: Decision, posts: Post[]): Promise<Outcome> {
...
async function main(): Promise<void> {
	...
	const decision = applyGuards(await decide(posts, log), posts, log);
	...
	const outcome = await execute(decision, posts);
	await appendLog(log, outcome.logEntry);

	if (DRY_RUN) {
		console.log(`[dry run] Files changed in working tree: ${outcome.changedFiles.join(', ')}`);
		console.log(`[dry run] Would commit: "${outcome.commitMessage}" and send Telegram:`);
